...
function onOpen() {
  ...
  ui.createMenu('MailChimp Menu')
    ...
    .addItem('List Growth Details','mailchimpListGrowth')
    .addToUi();
...
function mailchimpListGrowth() {
  ...
  try {
    ...
    // output the numbers to the sheet
    sheet.getRange(7,1,numRows,numCols).setValues(monthlyGrowth.reverse());
    
    // adds formulas for absolute and relative growth
    for (var i = 0; i < numRows; i++) {
      sheet.getRange(7+i,5).setFormulaR1C1('=iferror(R[0]C[-3] - R[-1]C[-3],0)');  // absolute monthly change in list
      sheet.getRange(7+i,6).setFormulaR1C1('=iferror((R[0]C[-4] - R[-1]C[-4])/R[-1]C[-4],0)').setNumberFormat("0.00%");  // rate of change in list
    }
